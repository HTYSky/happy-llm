...
>
> 1.  **ReLU 的“硬”截断**：
>     *   ReLU ($max(0, x)$) 非常简单粗暴：大于 0 的保留，小于 0 的直接一刀切变成 0。
>     *   **缺点**：在 0 点不可导，且小于 0 时梯度完全消失（Dead ReLU 问题），一旦神经元进入负区间，可能永远无法被激活。
> 2.  **GELU 的“软”结合**：
>     *   GELU 认为：**神经元的激活不应该只是看它是否大于0，还要看它在整体分布中的位置。**
>     *   公式本质是 $GELU(x) = x \cdot \Phi(x)$，其中 $\Phi(x)$ 是标准正态分布的累积分布函数。
>     *   **直观理解**：
>         *   当 $x$ 很大时（比如 $x=3$），它在正态分布中出现的概率 $\Phi(x) \approx 1$，所以输出 $\approx x$（保留）。
>         *   当 $x$ 很小时（比如 $x=-3$），概率 $\Phi(x) \approx 0$，所以输出 $\approx 0$（抑制）。
>         *   **关键区别**：在 0 附近，GELU 是**平滑非线性**的，且允许微小的负值输出。
>     *   **优势**：这种平滑性（Smoothness）让梯度传播更顺畅，有助于深层 Transformer 模型的收敛，因此在 BERT、GPT-3、LLaMA 等大模型中全面取代了 ReLU。
...
> **💡 核心概念对比：Encoder vs Decoder**
>
> 在 Transformer 家族中，模型通常分为三类，它们的区别主要在于**“能看到什么”**：
>
> 1.  **Encoder-only (如 BERT)**：
>     *   **特点**：**双向注意力**（Bidirectional Attention）。
>     *   **能力**：能同时看到上下文（左边和右边）。
>     *   **擅长**：**理解任务**（如文本分类、情感分析、实体识别）。因为它能通读全文，理解最深刻。
>     *   **类比**：像做**阅读理解**，可以反复看前看后。
>
> 2.  **Decoder-only (如 GPT 系列)**：
>     *   **特点**：**单向注意力**（Causal/Masked Attention）。
>     *   **能力**：只能看到上文（左边），看不到未来（右边）。
>     *   **擅长**：**生成任务**（如写作、聊天）。因为它模拟了人类说话的顺序。
>     *   **类比**：像**即兴演讲**，说出口的话不能收回，只能根据已说的推测下一句。
>
> 3.  **Encoder-Decoder (如 T5, BART)**：
>     *   **特点**：结合了两者。Encoder 理解输入，Decoder 生成输出。
>     *   **擅长**：**序列到序列任务**（如翻译、摘要）。
>
> **总结**：BERT 选择了 Encoder，因为它想做“语言理解专家”；GPT 选择了 Decoder，因为它想做“语言生成专家”。
...
因此，预训练数据的核心要求即是需要极大的数据规模（数亿 token）。毫无疑问，通过人工标注产出的全监督数据很难达到这个规模。因此，预训练数据一定是从无监督的语料中获取。这也是为什么传统的预训练任务都是 LM 的原因。

> **💡 术语解释：LM (Language Model)**
>
> 这里的 **LM** 特指 **Standard Language Model（标准语言模型）**，也常被称为 **Causal Language Model (CLM，因果语言模型)**。
> *   **核心任务**：根据上文预测下一个词（Next Token Prediction）。
> *   **例子**：给定“今天天气”，预测“真好”。
> *   **特点**：单向（只能看左边），是 GPT 系列模型的核心训练方式。
>
> 这里的上下文是在对比 BERT 的 MLM（双向）和传统的 LM（单向）。
...
由于模型可以利用被遮蔽的 token 的上文和下文一起理解语义来预测被遮蔽的 token，因此通过这样的任务，模型可以拟合双向语义，也就能够更好地实现文本的理解。

> **💡 深度释疑：名字叫“编/解码器”，为什么区别却在“掩码”上？**
>
> 这是一个非常深刻的问题。**“Encoder/Decoder”是功能上的名字，而“掩码（Mask）”是实现这个功能的手段。**
>
> 1.  **Encoder (编码器) —— 任务是“理解”**
>     *   **需求**：要理解“苹果”是指水果还是手机，必须同时看它的上文（“吃了一个”）和下文（“真甜”）。
>     *   **手段**：**不使用因果掩码（No Causal Mask）**。它的注意力机制是**全开放**的，谁都能看谁。
>     *   **你提到的“随机 Mask”**：这其实不是它的“生理构造”，而是给它出的**考题**（MLM 任务）。就像考试时故意把卷子涂黑几个字让你填，但你的眼睛（Attention）本身是能看全卷子的。
>
> 2.  **Decoder (解码器) —— 任务是“生成”**
>     *   **需求**：说话是一个字一个字往外蹦的。在说第一个字的时候，绝对不能偷看第二个字（因为还没生成出来）。
>     *   **手段**：**强制使用上三角掩码（Causal Mask）**。
>     *   **这是“生理构造”**：物理上就禁止了它看右边（未来）。
>
> **一句话总结**：
> *   **Encoder** = **全向视野** (架构) + **随机挖空** (训练题目)。
> *   **Decoder** = **单向视野** (架构) + **预测下一个词** (训练题目)。
...
> **💡 深度释疑：GPT 只有 Decoder，怎么理解用户的问题？**
>
> 这是一个非常经典的疑问。既然 Decoder 只能“看左边”，那它怎么理解用户输入的 Prompt 呢？
>
> 1.  **Decoder 也能“回头看”**：
>     *   虽然 Decoder 在生成时不能看**未来**（还没生成的词），但它完全可以看**历史**（用户已经输入的 Prompt）。
>     *   对于 GPT 来说，用户的输入就是“历史”。当它开始生成回答的第一个字时，Attention 机制会把用户输入的所有字都看一遍，提取特征。
>
> 2.  **“理解”是“生成”的前提**：
>     *   为了准确预测下一个字（Next Token Prediction），模型被迫去压缩和理解之前所有字的信息。
>     *   例如，要补全“法国的首都是____”，模型必须先理解“法国”和“首都”这两个概念的关系，才能填出“巴黎”。
>     *   这种**“通过生成来理解”**的方式，在大规模数据下被证明非常有效。
>
> 3.  **架构统一的优势**：
>     *   Encoder-Decoder 架构（如 T5）需要两个不同的组件，参数利用率可能不如纯 Decoder 高。
>     *   GPT 这种“一条道走到黑”的架构更简单、更利于大规模扩展（Scaling Law），这也是为什么现在的大模型大多采用 Decoder-only 架构。

> **💡 追问：既然 Decoder 也能看历史，为什么还需要 Encoder？**
>
> 这是一个非常敏锐的问题！既然 Decoder 也能看历史，那 Encoder 存在的意义是什么呢？
>
> 1.  **核心差异：能不能看“未来”**
>     *   **Decoder (单向)**：读到第 5 个字时，绝对看不到第 6 个字。
>     *   **Encoder (双向)**：读到第 5 个字时，能同时看到第 1-10 个字。
>     *   **例子**：“因为**它**很红，所以这个**苹果**很好吃。”
>         *   **Decoder**：读到“它”时，还不知道后面是“苹果”，只能瞎猜。
>         *   **Encoder**：读到“它”时，已经看到了后面的“苹果”，能瞬间知道“它=苹果”。
>
> 2.  **应用场景不同**：
>     *   **Encoder**：适合**“事后分析”**（比如文本分类、情感分析），因为此时句子已经写完了，当然要看全。
>     *   **Decoder**：适合**“实时生成”**，因为说话时不能预知未来。
>
> 3.  **现状**：
>     *   虽然 Encoder 在理解上更强，但现在的 LLM (Decoder-only) 通过把模型做大（Scaling Law）和海量数据训练，已经能通过“看很多历史”来弥补“不看未来”的劣势，所以 Encoder 逐渐式微。
...
> **💡 知识拓展：模型是怎么“选”出下一个词的？（解码策略）**
>
> 模型输出的其实是下一个词的**概率分布**（比如：猫=0.6，狗=0.3，鸟=0.1）。如何根据这个概率选出最终的词，就是**解码策略**：
>
> 1.  **贪婪解码 (Greedy Decoding)**：
>     *   **策略**：每次都选概率**最大**的那个词（只看眼前）。
>     *   **优点**：计算最快，逻辑最稳。
>     *   **缺点**：容易陷入死循环（复读机），且生成的句子可能整体不通顺（因为局部最优 $\neq$ 全局最优）。
>
> 2.  **集束搜索 (Beam Search)**：
>     *   **策略**：每次多留几个备胎（比如保留前 3 个），走几步后再比较哪条路整体概率最高。
>     *   **优点**：比贪婪更稳健，生成的句子质量更高。
>     *   **缺点**：计算量大。
>
> 3.  **随机采样 (Sampling, Top-k, Top-p)**：
>     *   **策略**：不一定选最大的，而是根据概率**随机**抽一个。
>     *   **优点**：生成的文本更有**创造性**和多样性（ChatGPT 默认就是这种）。
...
GLM 最初是由清华计算机系推出的一种通用语言模型基座，其核心思路是在传统 CLM (Causal Language Model，因果语言模型) 预训练任务基础上，加入 MLM (Masked Language Model，掩码语言模型) 思想，从而构建一个在 NLG (Natural Language Generation，自然语言生成) 和 NLU (Natural Language Understanding，自然语言理解) 任务上都具有良好表现的统一模型。
