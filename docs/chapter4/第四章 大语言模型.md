...
在第一章中，我们已经介绍了**语言模型的概念，即通过预测下一个 token 任务来训练的 NLP 模型**。LLM 使用与传统预训练语言模型相似的架构与预训练任务（如 Decoder-Only 架构与 CLM 预训练任务），但拥有更庞大的参数、在更海量的语料上进行预训练，也从而展现出与传统预训练语言模型截然不同的能力。
...
> **💡 深度解析：旋转位置编码（RoPE）到底是什么？**
>
> RoPE 是目前 LLM（如 LLaMA、Qwen、GLM）最常用的位置编码方式,它解决了传统位置编码的两大痛点。
>
> **1. 传统位置编码的问题**
> *   **Transformer 原始方案（Sinusoidal）**：通过三角函数给每个位置一个固定的向量,直接加到词向量上。
>     *   **缺点**：位置信息和语义信息"混"在一起,且无法很好地处理超长文本（外推能力差）。
> *   **BERT 的可学习位置编码**：每个位置对应一个可训练的向量。
>     *   **缺点**：训练时是 512 长度,推理时就完全无法处理 513+ 长度的文本（没学过）。
>
> **2. RoPE 的核心思想：用"旋转"表达位置**
> *   不直接给词向量"加"位置信息,而是在注意力计算时,通过**旋转 Query 和 Key 向量**来注入位置关系。
> *   **数学本质**：把高维向量看成多个 2D 平面的组合,在每个平面上根据位置进行旋转（类比钟表指针）。
> *   **公式**（简化版）：对于位置 $m$ 的 Query/Key 向量,将其第 $i$ 维和第 $i+1$ 维构成的 2D 向量旋转 $m\theta_i$ 角度,其中 $\theta_i = 10000^{-2i/d}$。
>
> **3. RoPE 的两大优势**
> *   **相对位置建模**：
>     *   通过旋转角度差 $(m-n)\theta$ 来表达位置 $m$ 和位置 $n$ 之间的相对关系。
>     *   模型关注的是"这两个词相距多远",而不是"它们各自在第几位",这更符合语言规律。
> *   **长度外推能力**：
>     *   由于旋转是周期性的（像钟表）,即使推理时遇到训练时没见过的长度（比如训练 4k,推理 32k）,模型依然能"猜"出位置关系。
>     *   例如：InternLM 在 32k 长度上训练,通过 RoPE 能推理 200k 长度。
>
> **4. 直观类比**
> *   **传统方法**：给每个词贴一个"门牌号"标签（位置 1、2、3...）。
> *   **RoPE**：给每个词一个"钟表指针",位置不同指针角度就不同,两个词的相对位置就看指针的夹角。

例如,InternLM 在 32k 长度上下文上进行了预训练,但通过 RoPE 能够实现 200k 长度的上下文处理。通过不断增强长文本处理能力,LLM 往往能够具备更强的信息阅读、信息总结能力,从而解决诸如要求 LLM 读完《红楼梦》并写一篇对应的高考作文的"世纪难题"。
...
更重要的是，LLM 往往需要使用更大规模的预训练语料。根据由 OpenAI 提出的 Scaling Law：C ~ 6ND，其中 C 为计算量，N 为模型参数，D 为训练的 token 数，可以实验得出**训练 token 数应该是模型参数的 1.7倍**，也就是说 175B 的 GPT-3，需要使用 300B token 进行预训练。而 LLaMA 更是进一步提出，使用 20倍 token 来训练模型能达到效果最优，因此 175B 的 GPT-3，可以使用3.5T token 数据预训练达到最优性能。
...
> **💡 深度解析：KL散度（KL Divergence）是什么？**
>
> **KL散度**，全称 **Kullback-Leibler Divergence**，也叫**相对熵（Relative Entropy）**，是衡量两个概率分布差异程度的指标。
>
> **1. 数学定义**
...
>
> **2. 直观理解：衡量"惊讶程度"**
> *   假设 $P$ 是真实分布，$Q$ 是我们估计的分布。
> *   KL散度衡量的是：如果用 $Q$ 来编码本该用 $P$ 编码的数据，会浪费多少额外信息。
> *   **类比**：
>     *   你以为明天下雨概率 90%（$Q$），实际上只有 10%（$P$）。
>     *   你带了一堆雨具出门（根据 $Q$ 做决策），结果完全不需要。
>     *   这种"决策偏差"就是 KL散度要衡量的。
>
> **3. 核心特性**
> *   **非对称性**：$D_{KL}(P||Q) \neq D_{KL}(Q||P)$（这不是距离，而是"有方向的差异"）。
> *   **非负性**：$D_{KL}(P||Q) \geq 0$，当且仅当 $P=Q$ 时等于 0。
> *   **值越大，差异越大**：KL散度越接近0，两个分布越相似。
>
> **4. 在RLHF中的作用：防止"遗忘"**
> *   **Actor Model**（$\pi_{PPO}$）：正在训练的模型，会不断更新参数。
> *   **Ref Model**（$\pi_{base}$）：冻结的参考模型，参数不变。
> *   **问题**：如果放任 Actor 自由更新，它可能为了讨好 Reward Model，生成一些极端的、毫无意义的回复（比如不停重复"好的好的好的"来获得高分）。
> *   **KL散度的约束**：$r_{KL} = -\theta_{KL}D_{KL}(\pi_{PPO}||\pi_{base})$
>     *   当 Actor 输出的分布和 Ref 差异太大时，KL散度变大，惩罚项 $r_{KL}$ 变小（因为前面有负号）。
>     *   这就像给模型套了个"橡皮筋"，让它不要离原来的能力太远，既要进步，又不能忘掉预训练和SFT学到的知识。
>
> **5. 举例说明**
> *   **Ref Model 输出**："我建议你可以……"（正常回复）
> *   **Actor Model 输出情况1**："我建议你可以试试……"（微调后，稍有改进）→ KL散度很小 ✅
> *   **Actor Model 输出情况2**："好的好的好的好的……"（为了高分乱答）→ KL散度很大 ❌
>
> **总结**：KL散度在RLHF中就像一个"安全绳"，确保模型在强化学习优化的过程中，不会过度偏离原来的知识基础，从而保持输出的质量和可靠性。
